...
class TestBuildSortedScores:
    def test_returns_sorted_array(self):
        from hackernews_simulator.model.calibrate import build_sorted_scores
...
class TestComputeTimeStats:
    def test_hourly_stats_has_24_entries(self):
        from hackernews_simulator.model.calibrate import compute_time_stats
        df = pd.DataFrame({
            "time": pd.to_datetime([
...
class TestSaveLoadCalibrationData:
    def test_roundtrip_sorted_scores(self, tmp_path):
        from hackernews_simulator.model.calibrate import save_sorted_scores, load_sorted_scores
    ...
    def test_roundtrip_time_stats(self, tmp_path):
        from hackernews_simulator.model.calibrate import save_time_stats, load_time_stats
